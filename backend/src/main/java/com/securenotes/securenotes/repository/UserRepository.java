...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    List<User> findAllAfterId(@Param("afterId") Long afterId);

    // Find users by partial role match (case-insensitive)
    @Query("SELECT u FROM User u WHERE LOWER(u.role) LIKE LOWER(CONCAT('%', :rolePart, '%'))")
    List<User> searchByRole(@Param("rolePart") String rolePart);

    // Find users who are not locked and have failed login attempts
    List<User> findByLockedFalseAndFailedLoginAttemptsGreaterThan(int threshold);

    // Find users by username or email (if you add email field)
    // List<User> findByUsernameOrEmail(String username, String email);

    // Get only user IDs for all users
